...
namespace MultipleDelegation_MultipleConnector_P_P_Operation
{
	public class testPExecution : OpaqueBehaviorExecution
	{
		...
		{
			types.Add(MultipleDelegation_MultipleConnector_P_P_OperationModel.Instance().MultipleDelegation_MultipleConnector_P_P_Operation_main_testP);
		...
		public override void DoBody(List<ParameterValue> inputParameters, List<ParameterValue> outputParameters)
		{
			if(index == 1)
	{
		Console.WriteLine("Asserting c.b1.p == 4 || c.b2.p == 4 || c.b3.p == 4 || c.b4.p == 4");
	}
	...
	Console.WriteLine( "c.b" + index + ".p == " + p);
	success = success || (p == 4);
